...
STATUS = {
    0: ('В главном меню', 'In main menu'),
    1: ('Смотрит повтор', 'Watching a replay'),
    2: ('За рулем', 'Live'),
    3: ('Пауза', 'In pause')
}
...
RAIN = {
    0: ('Без дождя', 'No Rain'),
    1: ('Изморось', 'Drizzle'),
    2: ('Слабый дождь', 'Light Rain'),
    3: ('Дождь', 'Rain'),
    4: ('Ливень', 'Heavy Rain'),
    5: ('Ураган', 'Thunderstorm')
}
SESSION = {
    -1: ('Неизвестно', 'Unknown'),
    0: ('Практика', 'Practice'),
    1: ('Квалификация', 'Qualify'),
    2: ('Гонка', 'Race'),
    3: ('Индивидуальный круг', 'Hotlap'),
    4: ('Time Attack', 'Time Attack'),
    5: ('Дрифт', 'Drift'),
    6: ('Драг', 'Drag'),
    7: ('Хотстинт', 'Hotstint'),
    8: ('Суперпол хотлап', 'Hotlapsuperpole')
}
...
MSG = [
    ('Автор', 'Author'),
    ('Спасибо за использование программы. Она все еще находится в разработке.', 'Thanks for using the program, it is still under development.'),
    ('Определяем язык вашей системы..', 'Checking your system language ..'),
    ('Язык обнаружен:', 'Language detected:'),
    (f'Ожидание запуска {GAME}..', f'Waiting for {GAME} process ..'),
    (f'Процесс {GAME} обнаружен', f'{GAME} process detected'),
    ('Подключение к Discord..', 'Connecting with Discord ..'),
    ('Успешное подключение к Discord', 'Discord connected successfully'),
    ('Одиночная игра', 'Singleplay'),
    ('Многопользовательская игра', 'Multiplayer'),
    ('Время', 'Time'),
    ('Позиция', 'Position'),
    ('На пит-стопе', 'In the pits'),
    ('Кругов завершено', 'laps completed'),
    ('В меню настроек', 'In configuration menu'),
    ('Поиск обновлений ..', 'Checking updates ..'),
    ('Доступно новое обновление, пожалуйста, обновите до версии', 'New update is available, please update to version'),
    ('Обновлено', 'Up to date')
]

ERROR = [
    ('Программа прервана пользователем', 'Program interrupt by user'),
    ('Язык не обнаружен', 'Language not detected'),
    ('Превышен лимит повторного подключения к Discord, программа будет закрыта', 'Discord reconnection limit exceeded, closing program ..'),
    ('Ошибка при подключении к Discord', 'An error occurred when trying to connect'),
    ('Повторное подключение к Discord через 5с ..', 'Reconnecting with Discord in 5s ..'),
    (f'{GAME} закрыта, подключение ..', f'{GAME} has closed, reconnecting ..'),
    ('Ошибка при работе с файлом регистрации', 'Error on generating log file')
]
